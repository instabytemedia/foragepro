...
**Product:** ForagePro
**Entities with Forms:** Idea, User, Recipe
...
Create **RecipeForm Component** (`components/recipe/RecipeForm.tsx`):

**Props Interface:**
- `recipe?`: Recipe Object (if provided = Edit Mode)
- `onSuccess?`: Callback after successful submit

**Functionality:**
- Client Component ("use client")
- react-hook-form with zodResolver
- Use CreateRecipeSchema for Validation
- Distinguish Create vs Edit Mode (based on recipe prop)
- useCreateRecipe() Hook for Create
- useUpdateRecipe(id) Hook for Edit
- Loading State during Submit
- Error Handling with Alert
- After Success: onSuccess Callback + Router Push to List + Router Refresh
...
**Layout:**
- Card Container
- CardHeader: Title ("New Recipe" or "Edit Recipe")
- CardContent: Form Fields (space-y-4)
- CardFooter: Actions (justify-end gap-2)
  - Cancel Button (variant="outline", onClick → router.back())
  - Submit Button (type="submit", disabled during Loading)
...
**Important:**
- register() with valueAsNumber for Number fields
- defaultValues from recipe prop (for Edit Mode)
- Proper Error Display under each field

---

Create **New Recipe Page** (`app/(app)/recipes/new/page.tsx`):
