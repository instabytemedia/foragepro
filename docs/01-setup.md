...
**Solution:** Social marketplace platform connecting enthusiasts with local experts and suppliers
...
export function Logo({ size = "default" }: { size?: "sm" | "default" | "lg" }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={`font-bold tracking-tight ${s.text}`}>ForagePro</span>
    </div>
  );
}
```
